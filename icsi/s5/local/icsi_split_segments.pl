...
while(<S>) {
  ...
  my @A = split(" ", $_);
  if (@A < 6) { print "Skipping line @A"; next; }
  
  my ($meet_id, $channel, $spk, $aut_btime, $aut_etime) = @A[0..4];
  my @transcript = @A[5..$#A];
  my %transcript = split_transcripts(\@transcript, $aut_btime, $aut_etime, 30); 
  my @meetid = split(/\./, $meet_id);
  ...
  for my $key (keys %transcript) {
    ...
    if (length($segment)>0) {
       print W join " ", $meetid[0], $channel, $spk, $times[0]/100.0, $times[1]/100.0, $segment, "\n";
    }
  }
