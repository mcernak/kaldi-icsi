...
if [ $stage -le 8 ]; then
  ...
  steps/nnet2/train_multisplice_accel2.sh --stage $train_stage \
    ...
    --mix-up 12000 \
    data/ihm/train_hires data/lang exp/ihm/tri3_ali $dir  || exit 1;
fi
# exit
if [ $stage -le 9 ]; then
...
if [ $stage -le 11 ]; then
  ...
  lm_suffix=ami.o3g.kn.pr1-7
  graph_dir=exp/ihm/tri3/graph_${lm_suffix}
  steps/online/nnet2/decode.sh --cmd "$decode_cmd" --nj 8 \
    "$graph_dir" data/ihm/eval ${dir}_online/decode_${lm_suffix} || exit 1;
fi
exit
if [ $stage -le 12 ]; then
